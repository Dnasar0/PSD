...
def generate_dh_keys(parameters):
    # Gerar chave privada e pública Diffie-Hellman
    private_key = parameters.generate_private_key()
    public_key = private_key.public_key()
    return private_key, public_key

def serialize_public_key(public_key):
    # Serializar chave pública para compartilhamento
...
def load_public_key(pem):
    # Carregar chave pública recebida
...
def derive_shared_key(private_key, peer_public_key):
    ...
    # Derivar uma chave AES a partir da chave compartilhada
    derived_key = HKDF(
...
# Função para gerar chaves privada e pública Diffie-Hellman
def generate_dh_private_public_key(parameters):
    private_key = parameters.generate_private_key()
    public_key = private_key.public_key()
    return private_key, public_key

# Função para calcular a chave partilhada Diffie-Hellman
...
# Função para encriptar mensagens
def encrypt_message(message, aes_key):
    cipher = Cipher(algorithms.AES(derive_shared_key(private_key, peer_public_key)), modes.GCM(), backend=default_backend())
    encryptor = cipher.encryptor()
    ciphertext = encryptor.update(message.encode()) + encryptor.finalize()
    return (encryptor.tag, ciphertext)

# Função para desencriptar mensagens
def decrypt_message(encrypted_message, aes_key, tag):
    cipher = Cipher(algorithms.AES(derive_shared_key(private_key, peer_public_key)), modes.GCM(tag), backend=default_backend())
